...
* `tile_annotations(annotations: List[Union[BoundingBox, Keypoint]], image_width: int, image_height: int, slice_width: int, slice_height: int, horizontal_overlap_ratio: float, vertical_overlap_ratio: float) -> List[List]`:
    Tiles annotations based on the image tiling generated by `tile_image`.

* `get_annotations_in_tile(annotations: List[Union[BoundingBox, Keypoint]], tile: Tuple[int, int, int, int]) -> List`:
    Filters annotations and returns only those that completely intersect with a given tile.
